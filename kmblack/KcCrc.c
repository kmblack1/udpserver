...
//uint16_t SEAPI KcCrc16(const char *buf, uint16_t len) {
...
uint16_t kcCrc16(const uint8_t* data, size_t offset, size_t len, uint16_t preval) {
	static const uint8_t KC_CRC16_TAB_HIG[] = {
	...
	static const uint8_t KC_CRC16_TAB_LOW[] = {
	...
	uint16_t uCRCHi = (preval & 0xff00) >> 8;
	uint16_t uCRCLo = preval & 0x00ff;
	size_t iIndex;
	for (size_t i = 0; i < len; ++i) {
		iIndex = (uCRCLo ^ data[offset + i]) & 0x00ff;
		uCRCLo = uCRCHi ^ KC_CRC16_TAB_HIG[iIndex];
		uCRCHi = KC_CRC16_TAB_LOW[iIndex];
	}
	return((uCRCHi & 0x00ff) << 8) | (uCRCLo & 0x00ff) & 0xffff;
}


uint32_t kcCrc32(const char* buf, size_t offset, size_t len) {
	...
	for (i = 0; i < len; ++i)
		crc32val = crc32_tab[(crc32val ^ buf[offset + i]) & 0xFF] ^ ((crc32val >> 8) & 0x00FFFFFF);
	return crc32val ^ 0xFFFFFFFF;
}



uint64_t  kcCrc64(const char* buf, size_t offset, size_t len) {
